...
public class MultiStateView extends FrameLayout {
    ...
    private static final int ERROR_NETWORK_VIEW = 4;

    private static final int NO_CONTENT_VIEW = 5;
    ...
    private void init(AttributeSet attrs) {
        ...
        int errorNetWorkViewResId = a.getResourceId(R.styleable.MultiStateView_msv_errorNetworkView, -1);
        if (errorNetWorkViewResId > -1) {
            mErrorNetworkView= mInflater.inflate(errorNetWorkViewResId, this, false);
            addView(mErrorNetworkView, mErrorNetworkView.getLayoutParams());
        }


        int noContentViewResId = a.getResourceId(R.styleable.MultiStateView_msv_noContentView, -1);
        if ( noContentViewResId > -1) {
            mNoContentView= mInflater.inflate(noContentViewResId, this, false);
            addView(mNoContentView, mNoContentView.getLayoutParams());
        }
        ...
        if (viewState != UNKNOWN_VIEW) {
            switch (viewState) {
                ...
                case ERROR_NETWORK_VIEW:
                    mViewState = ViewState.ERROR_NETWORK;
                    break;
                case NO_CONTENT_VIEW:
                    mViewState = ViewState.NO_CONTENT_VIEW;
                    break;
            }
        }
    ...
    private void setView() {
        switch (mViewState) {
            case LOADING:
                ...
                if (mErrorNetworkView != null) mErrorNetworkView.setVisibility(View.GONE);
                if (mNoContentView != null) mNoContentView.setVisibility(View.GONE);
                break;
            ...
            case ERROR_NETWORK:
                if (mErrorNetworkView == null) {
                    throw new NullPointerException("Error Network View");
                }

                mErrorNetworkView.setVisibility(View.VISIBLE);
                if (mLoadingView != null) mLoadingView.setVisibility(View.GONE);
                if (mContentView != null) mContentView.setVisibility(View.GONE);
                if (mEmptyView != null) mEmptyView.setVisibility(View.GONE);
                if (mErrorView != null) mErrorView.setVisibility(View.GONE);
    ...
    private boolean isValidContentView(View view) {
        ...
        return view != mLoadingView && view != mErrorView &&
                view != mErrorNetworkView && view != mEmptyView && view != mNoContentView;
    }
    ...
    public void setViewForState(View view, ViewState state, boolean switchToState) {
        switch (state) {
            ...
            case ERROR_NETWORK:
                if (mErrorNetworkView != null) removeView(mErrorNetworkView);
                mErrorNetworkView = view;
                addView(mErrorNetworkView);
                break;
            case NO_CONTENT_VIEW:
                if (mNoContentView != null) removeView(mNoContentView );
                mNoContentView  = view;
